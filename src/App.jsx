...
const analyzeVideo = (file) =>
  new Promise((resolve) => {
    ...
    video.onloadedmetadata = () => {
      ...
      const durationOk = duration >= MIN_DURATION_SEC;
      ...
      resolve({
        ...
        durationOk,
        resolutionOk,
        passed: durationOk && resolutionOk,
      });
    };
    video.onerror = () => {
      URL.revokeObjectURL(url);
...
export default function App() {
  ...
  const [qcResult, setQcResult] = useState(null);   // null | objeto con resultados
  ...
  // ── Upload a Cloudinary + guardar en Supabase ─────────────────────────────
  const handleUpload = () => {
    if (!formValid) return;
    setUploading(true);
    setUploadProgress(0);
    setUploadStage("Subiendo video a Cloudinary...");
    setError("");
    ...
    xhr.onload = async () => {
      if (xhr.status === 200) {
        ...
        setUploadStage("Guardando datos en la base de datos...");
        ...
        setTimeout(() => {
          ...
          setForm(EMPTY_FORM); setSuccess(true);
          setGeoError(""); setQcResult(null);
        }, 500);
  ...
  const QCPanel = () => {
    ...
    return (
      <div style={{
          ...
          <div>
            ...
            {!passed && (
              <div style={{ fontSize: "0.67rem", color: "#94A3B8", marginTop: 2 }}>
                Por favor corregí los puntos marcados con ❌ e intentá de nuevo.
              </div>
            )}
          </div>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", background: "#0A0E1A", borderRadius: 6, padding: "0.6rem 0.85rem" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "0.78rem", color: durationOk ? "#10B981" : "#EF4444", fontWeight: 700 }}>
                ...
                <span style={{ fontSize: "0.62rem", color: "#475569", fontWeight: 400, marginLeft: "0.5rem" }}>
                  (mínimo {formatDuration(MIN_DURATION_SEC)})
                </span>
              </div>
              {!durationOk && (
                <div style={{ fontSize: "0.65rem", color: "#94A3B8", marginTop: 2 }}>
                  El video es demasiado corto. Grabá al menos {MIN_DURATION_SEC} segundos para poder aplicar los algoritmos de velocimetría.
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", background: "#0A0E1A", borderRadius: 6, padding: "0.6rem 0.85rem" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "0.78rem", color: resolutionOk ? "#10B981" : "#EF4444", fontWeight: 700 }}>
                {width}x{height}px
                <span style={{ fontSize: "0.62rem", color: "#475569", fontWeight: 400, marginLeft: "0.5rem" }}>
                  (mínimo {MIN_WIDTH}x{MIN_HEIGHT}px — 720p)
                </span>
              </div>
              {!resolutionOk && (
                <div style={{ fontSize: "0.65rem", color: "#94A3B8", marginTop: 2 }}>
                  La resolución es demasiado baja. Usá una cámara con resolución mínima de 720p (1280x720).
                </div>
              )}
            </div>
          </div>
        ...
        {passed && (
          <div style={{ marginTop: "0.75rem", fontSize: "0.63rem", color: "#475569", background: "#0A0E1A", borderRadius: 4, padding: "0.5rem 0.75rem" }}>
            ℹ️ El análisis de estabilidad de cámara se realizará durante el procesamiento con RIVeR.
          </div>
        )}
      </div>
    );
    ...
    <div style={{ fontFamily: "'Courier New', monospace", background: "#0A0E1A", minHeight: "100vh", color: "#E2E8F0" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cancel-btn { background: none; border: 1px solid #EF4444; border-radius: 4px; color: #EF4444; cursor: pointer; font-family: 'Space Mono', monospace; font-size: 0.65rem; letter-spacing: 0.08em; padding: 0.5rem 1rem; transition: all 0.2s; }
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.3rem" }}>
            ...
            <div style={{ background: "#0F172A", border: "1px solid #1E293B", borderRadius: 8, padding: "0.85rem 1.1rem" }}>
              ...
              <div style={{ display: "flex", gap: "1.5rem", flexWrap: "wrap" }}>
                <span style={{ fontSize: "0.68rem", color: "#94A3B8" }}>⏱ Duración mínima: <strong style={{ color: "#CBD5E1" }}>10 segundos</strong></span>
                <span style={{ fontSize: "0.68rem", color: "#94A3B8" }}>📐 Resolución mínima: <strong style={{ color: "#CBD5E1" }}>720p (1280x720)</strong></span>
                <span style={{ fontSize: "0.68rem", color: "#94A3B8" }}>🎥 Formatos: <strong style={{ color: "#CBD5E1" }}>MP4, MOV, AVI, MKV</strong></span>
              </div>
            </div>
            ...
            >
              ...
              {preview ? (
                <div onClick={e=>e.stopPropagation()}>
                  ...
                  <div style={{ marginTop: "0.6rem", fontSize: "0.7rem", color: "#38BDF8" }}>
                    📹 {selectedFile?.name} · {(selectedFile?.size/1e6).toFixed(1)} MB
                  </div>
                  {!uploading && (
                    <button style={{ marginTop: "0.4rem", background: "none", border: "none", color: "#475569", fontSize: "0.62rem", cursor: "pointer" }} onClick={e=>{e.stopPropagation();setSelectedFile(null);setPreview(null);setQcResult(null);}}>✕ CAMBIAR VIDEO</button>
                  )}
                </div>
              ) : (
                <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div className="grid2">
                  <div>
                    ...
                    <input type="date" className="field-input" value={form.date} onChange={e=>setF("date",e.target.value)} disabled={uploading} />
                  </div>
                  <div>
                    <label className="field-label">Hora de la captura <span className="req">*</span></label>
                    <input type="time" className="field-input" value={form.time} onChange={e=>setF("time",e.target.value)} disabled={uploading} />
                  </div>
                </div>
                <div className="grid2">
                  <div>
                  ...
                  <div>
                    ...
                    <input className="field-input" placeholder="ej: Villa Urquiza, Los Pocitos..." value={form.locality} onChange={e=>setF("locality",e.target.value)} disabled={uploading} />
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap" }}>
                    ...
                    <button className="geo-btn" disabled={geoLoading || uploading} onClick={getGeolocation}>
            ...
            <div className="card" style={{ padding: "1.35rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                ...
                <div>
                  ...
                  <input className="field-input" placeholder="ej: María García" value={form.name} onChange={e=>setF("name",e.target.value)} disabled={uploading} />
                </div>
                <div>
            ...
            {uploading ? (
              <div className="card" style={{ padding: "1.35rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.7rem" }}>
                  <div style={{ fontSize: "0.68rem", color: "#38BDF8", letterSpacing: "0.1em" }}>
                ...
                <div style={{ fontSize: "0.62rem", color: "#334155", marginTop: "0.5rem", textAlign: "center" }}>
                  No cierres esta pestaña mientras se sube el video
                </div>
              </div>
            ) : (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.75rem" }}>
                <div style={{ fontSize: "0.64rem", color: "#334155" }}>
                  <span className="req">*</span> Obligatorio: video aprobado · fecha · hora · localidad · contacto
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.3rem" }}>
            <div className="about-card">
              ...
              <p style={{ fontSize: "0.75rem", color: "#94A3B8", lineHeight: 1.8 }}>
                La <strong style={{color:"#CBD5E1"}}>velocimetría por imágenes</strong> es una familia de técnicas no intrusivas que permiten medir la velocidad del flujo de agua a partir de secuencias de video. Al rastrear patrones naturales en la superficie, los algoritmos calculan los vectores de velocidad sin necesidad de instrumentos en contacto con el agua. Los videos son procesados con <strong style={{color:"#CBD5E1"}}>RIVeR (Rapid Image Velocimetry and Ranging)</strong>, implementando LSPIV para estimar velocidades superficiales en ríos.
              </p>
            </div>
            <div className="about-card">
              ...
              {[
                ["01","Control de calidad automático","Al seleccionar el video se verifica duración mínima (10 seg) y resolución mínima (720p) antes de permitir el envío."],
                ["02","Ingesta y georreferenciación","El video se sube a Cloudinary y los metadatos se guardan en la base de datos con hora y coordenadas GPS."],
                ["03","Revisión manual","El equipo técnico revisa el video, verifica la estabilidad de cámara y marca los 4 puntos de control para la ortorectificación."],
                ["04","Procesamiento con RIVeR","Se aplica LSPIV con corrección Unshake para estabilización, ortorectificación y cálculo de vectores de velocidad superficial."],
                ["05","Reporte de resultados","Mapa de vectores y velocidad superficial estimada (m/s) publicados en la plataforma."],
              ].map(([n,title,desc])=>(
            ...
            <div className="about-card">
              <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, fontSize: "0.9rem", letterSpacing: "0.1em", color: "#64748B", marginBottom: "0.85rem" }}>CONSEJOS PARA UN MEJOR VIDEO</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(185px,1fr))", gap: "0.65rem" }}>
                {[
                  ["📐","Vista cenital","Filmá desde arriba o ángulo pronunciado. Cámara fija si es posible."],
                  ["⏱","Mín. 10 segundos","30–60 s es ideal para promediar velocidades."],
                  ["💡","Buena iluminación","Luz natural. Evitá reflejos directos del sol."],
                  ["🚫","Sin zoom digital","Acercate físicamente para mejor calidad."],
                  ["📏","Escala de referencia","Un objeto conocido mejora la precisión del cálculo."],
                  ["🔒","Cámara estable","Usá trípode o apoyá el teléfono. El movimiento introduce error."],
                ].map(([e,t,d])=>(
                  <div key={t} style={{ background: "#0A0E1A", border: "1px solid #1E293B", borderRadius: 6, padding: "0.75rem" }}>
                    <div style={{ fontSize: 15, marginBottom: "0.3rem" }}>{e}</div>
                    <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, color: "#94A3B8", fontSize: "0.8rem", marginBottom: 2 }}>{t}</div>
                    <div style={{ fontSize: "0.65rem", color: "#475569", lineHeight: 1.6 }}>{d}</div>
                  </div>
                ))}
              </div>
            </div>
